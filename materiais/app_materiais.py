    ...
    try:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS erp_materiais (
                id SERIAL PRIMARY KEY, equipe_id TEXT, nome_material TEXT, codigo_sku TEXT,
                unidade_medida TEXT, custo_base REAL, taxa_refugo REAL, lead_time INTEGER, estoque_minimo REAL
            )
        ''')
        conexao.commit()
...
def api_salvar_material():
    ...
    try:
        ...
        if id_reg:
            ...
            ''', (nome_mat, cat_mat, un_medida, preco_un, estoque_seg, l_time, forn_padrao, refugo_pad, espec, id_reg, id_equipe))
        else:
            cursor.execute('''
                INSERT INTO erp_materiais (
                    equipe_id, nome_material, categoria, unidade_medida, preco_unitario,
                    estoque_seguranca, lead_time_entrega, fornecedor_padrao, coeficiente_refugo, especificacao_tecnica
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ''', (id_equipe, nome_mat, cat_mat, un_medida, preco_un, estoque_seg, l_time, forn_padrao, refugo_pad, espec))
