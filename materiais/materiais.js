...
window.carregarDadosIniciais = async function() {
    try {
        ...
        let orcamento = {
            ...
            custo_minuto_total: 0,
            custo_minuto_mao_obra: 0,
            custo_minuto_energia: 0,
            custo_minuto_insumo: 0,
            custo_minuto_depreciacao: 0,
            custo_minuto_ajuste: 0,
            minutos_disponiveis_mes: 13200
        };
...
window.renderizarQuadrosOperacionais = function(data) {
    ...
    if(ti) ti.innerHTML=instrumentos.length?instrumentos.map(i=>`<tr><td><strong>${escapeMateriais(i.nome_instrumento)}</strong></td><td>${escapeMateriais(i.categoria||'')}</td><td>${i.quantidade||1}</td><td>${moedaMateriais(numeroSeguro(i.preco_compra)*numeroSeguro(i.quantidade||1))}</td><td>${numeroSeguro(i.potencia_watts)*numeroSeguro(i.quantidade||1)} W<br>${numeroSeguro(i.consumo_gas_m3)*numeroSeguro(i.quantidade||1)} m³ gás</td><td>${moedaMateriais(i.insumo_minuto)}/min insumo<br>${moedaMateriais(i.custo_minuto)}/min ajuste</td><td>${i.is_patrimonio?'Sim':'Não'}</td><td><button class="btn-top" type="button" onclick="window.deletarInstrumentoMateriais(${i.id})">Remover</button></td></tr>`).join(''):`<tr><td colspan="8" style="padding:16px;text-align:center;font-style:italic;">Nenhum instrumento/material de apoio registrado.</td></tr>`;
    ...
    if(tm) tm.innerHTML=instrumentos.length?instrumentos.map(i=>{
        const qtd=Math.max(1,numeroSeguro(i.quantidade));
        const mins=Math.max(1,numeroSeguro(i.minutos_operacionais_mes)||13200);
        const dep=numeroSeguro(i.preco_compra)*qtd/(Math.max(1,numeroSeguro(i.depreciacao_anos)||10)*12*mins);
        const mao=numeroSeguro(resumo.custo_minuto_mao_obra);
        const energiaMin=numeroSeguro(resumo.custo_minuto_energia);
        const insumo=numeroSeguro(i.insumo_minuto)*qtd;
        const ajuste=numeroSeguro(i.custo_minuto);
        const total=dep+mao+energiaMin+insumo+ajuste;
        return `<tr><td><strong>${escapeMateriais(i.nome_instrumento)}</strong></td><td>${escapeMateriais(i.categoria||'')}</td><td>${qtd}</td><td>${(numeroSeguro(i.potencia_watts)*qtd).toLocaleString('pt-BR')} W</td><td>${moedaMateriais(dep)}/min</td><td>${moedaMateriais(mao)}/min</td><td>${moedaMateriais(energiaMin)}/min</td><td>${moedaMateriais(insumo)}/min</td><td>${moedaMateriais(ajuste)}/min</td><td><strong>${moedaMateriais(total)}/min</strong></td></tr>`;
    }).join(''):`<tr><td colspan="10" style="padding:16px;text-align:center;font-style:italic;">Nenhum equipamento de apoio registrado.</td></tr>`;
    const totalMin=numeroSeguro(resumo.custo_minuto_total);
    const k=document.getElementById('kpi_custo_minuto_materiais'); if(k) k.textContent=moedaMateriais(totalMin)+'/min';
    const c=document.getElementById('kpi_custo_minuto_composicao'); if(c) c.innerHTML=`Mão de obra: ${moedaMateriais(resumo.custo_minuto_mao_obra)} &nbsp;|&nbsp; Energia: ${moedaMateriais(resumo.custo_minuto_energia)} &nbsp;|&nbsp; Insumo: ${moedaMateriais(resumo.custo_minuto_insumo)} &nbsp;|&nbsp; Depreciação: ${moedaMateriais(resumo.custo_minuto_depreciacao)} &nbsp;|&nbsp; Ajuste: ${moedaMateriais(resumo.custo_minuto_ajuste)}`;
};
