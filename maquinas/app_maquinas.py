...
@maquinas_blueprint.route("/api/maquinas/indicadores", methods=["GET"])
def api_indicadores_maquinas():
    """
    Retorna os indicadores consolidados do parque de máquinas e os custos
    fixos que já possuem fonte estruturada no banco.

    Importante: consumo de energia, água e gases é retornado como quantidade.
    Não existe tarifa unitária desses insumos na estrutura atual do banco;
    portanto, o módulo não inventa um valor monetário para eles.
    """
    if not autenticado():
        return jsonify({"status": "erro", "message": "Não autenticado."}), 401
    ...
    def soma(cur, tabela, expressao):
        if not tabela_existe(cur, tabela):
            return 0.0
        cur.execute(
            f"SELECT COALESCE(SUM({expressao}), 0) AS total FROM {tabela} WHERE equipe_id = %s",
            (equipe_atual(),),
        )
        row = cur.fetchone()
    ...
    try:
        ...
        # Recursos físicos cadastrados no módulo Máquinas.
        cursor.execute(
            """
            SELECT
                COALESCE(SUM(consumo_eletrico), 0) AS eletrico,
                COALESCE(SUM(consumo_agua), 0) AS agua,
                COALESCE(SUM(consumo_gases), 0) AS gases,
                COALESCE(SUM(depreciacao_mensal), 0) AS depreciacao
            FROM erp_maquinas
            WHERE equipe_id = %s
        ...
        maquinas = cursor.fetchone() or {}

        folha_fixa = soma(cursor, "folha_funcionarios", "COALESCE(salario_base, 0)")
        rh_fixo = soma(cursor, "estrutura_rh", "COALESCE(subtotal, 0)")
        aluguel = soma(cursor, "imoveis_simulacao", "COALESCE(valor_aluguel, 0)")
        condominio = soma(cursor, "imoveis_simulacao", "COALESCE(valor_condominio, 0)")

        # O motor financeiro central é a referência para o custo fixo geral.
        try:
            import GerenciadorCaixa
            metricas = GerenciadorCaixa.calcular_metricas_totais_equipe(id_equipe)
            custo_fixo_empresa = float(metricas.get("custo_fixo_geral_empresa") or 0)
        except Exception:
            # Fallback compatível com as fontes explicitamente disponíveis.
            custo_fixo_empresa = folha_fixa + rh_fixo + aluguel + condominio

        return jsonify({
            "status": "sucesso",
            "equipe_id": id_equipe,
            "consumo": {
                "eletrico": round(float(maquinas.get("eletrico") or 0), 4),
                "agua": round(float(maquinas.get("agua") or 0), 4),
                "gases": round(float(maquinas.get("gases") or 0), 4),
            },
            "custos_fixos": {
                "mao_obra_folha": round(folha_fixa, 2),
                "mao_obra_estrutura": round(rh_fixo, 2),
                "mao_obra": round(folha_fixa + rh_fixo, 2),
                "ocupacao": round(aluguel + condominio, 2),
                "depreciacao_maquinas": round(float(maquinas.get("depreciacao") or 0), 2),
                "total_empresa": round(custo_fixo_empresa, 2),
            },
        }), 200
        ...
        if conexao:
            conexao.rollback()
        logger.exception("Erro ao carregar indicadores de máquinas: %s", erro)
        return jsonify({
            "status": "erro",
            "message": "Não foi possível carregar os indicadores de Máquinas.",
            "erro": str(erro),
        }), 500
