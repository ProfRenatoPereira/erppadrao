...
pcp_blueprint = Blueprint('pcp_blueprint', __name__)

def obter_conexao_master():
    from app_master import URL_SUPABASE
    return psycopg2.connect(URL_SUPABASE)

@pcp_blueprint.route('/pcp')
def pagina_pcp():
    with open('pcp/pcp.html', 'r', encoding='utf-8') as f:
        html = f.read()
    return render_template_string(html)

@pcp_blueprint.route('/api/pcp/emitir', methods=['POST'])
def api_emitir_op():
    ...
    # Inicia a tabela de OPs do erppadrao
    cursor.execute('''
    ...
    # VERIFICAÇÃO PEDAGÓGICA (BOM): Puxa os insumos exigidos na ficha técnica do produto
    ...
    if produto:
        ...
        # Verifica se há saldo suficiente no Almoxarifado (Fase 5)
        ...
        if not material or float(material['quantidade_estoque']) < qtd_necessaria:
            cursor.close()
            conexao.close()
            return jsonify({'status': 'erro', 'message': f"Insumos insuficientes! Requer {qtd_necessaria} un do material, mas o saldo em Almoxarifado está baixo."}), 400
            
        # Abate os materiais consumidos do Almoxarifado de forma automática
...
@pcp_blueprint.route('/api/pcp/listar', methods=['GET'])
def api_listar_ops():
    if not session.get('logado'):
        return jsonify({'status': 'erro', 'message': 'Não autenticado'}), 401
    conexao = obter_conexao_master()
...
@pcp_blueprint.route('/api/pcp/finalizar/<int:id_reg>', methods=['POST'])
def api_finalizar_op_id(id_reg):
    if not session.get('logado'):
        return jsonify({'status': 'erro', 'message': 'Não autenticado'}), 401
    id_equipe = session.get('id_equipe', 'equipe_alfa')
    conexao = obter_conexao_master()
    ...
    if op:
        # 1. Altera status da OP para Finalizado
        cursor.execute('UPDATE ordens_producao SET status_op = \'Finalizado\' WHERE id = %s', (id_reg,))
        
        # 2. LOGÍSTICA DE DISTRIBUIÇÃO: Abastece o estoque de produtos acabados automaticamente
        # Verifica se o produto já tem registro de estoque
        ...
        if estoque:
            ...
            # Puxa o nome do catálogo para criar a nova posição de prateleira padrão
            ...
            cursor.execute('''
                ...
                VALUES (%s, %s, %s, %s, %s, 'Almoxarifado Central PCP', %s, 10)
